...
// M04-P2-R01 (HDA decision E): the Studio stage also carries the Sync module
// families the packer copies verbatim beside the Archive Workbench (Item 9
// adapters, P02 core, P02 Chrome adapters, local-publication adapters). Their
// expected artifact paths are derived from the same governed pack mappings that
// produce them, normalized to "/" artifact form, and each family is byte-compared
// against the authorized source root through its read-only pack helper. The
// file-set gate stays an exact expected-versus-actual membership comparison.
...
function studioSupplementalOutputFiles() {
  return STUDIO_SUPPLEMENTAL_FAMILIES.flatMap((entry) => entry.outputs.map(artifactRelativePath));
}
...
export function validateStagedStudioLauncher(stage, source, worktreeRoots) {
  ...
  // Every packed Sync module family must be byte-identical to the same
  // authorized source root (missing source, missing output or a byte delta all
  // reject; the family and its mismatched artifacts are reported).
  ...
  const foreignWorktrees = worktreeRoots.filter((root) => root !== approvedWorktree);
  for (const relative of actualFiles) {
    // ES modules (.mjs) are staged text too (M04-P2-R01).
