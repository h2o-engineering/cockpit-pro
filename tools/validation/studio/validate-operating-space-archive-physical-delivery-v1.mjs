...
//     lib.rs pre-image, mechanical-only lease scope) are only MEANINGFUL while
//     the tree is positioned as the T02 candidate. They run whenever that
//     position is detectable and otherwise report SKIP with the exact reason.
//     A stage-negative guard that asserts "the next stage has not happened"
//     goes red the moment it does, so these are deliberately scoped rather
//     than unconditional — the content fences above are what remain
//     permanently binding.
//
//   * PRODUCT-MAIN LIFECYCLE is checked whenever the frozen base is reachable,
//     including in later assurance runs. It is a WINDOW, not a hash pin:
//     Window A is main still at the frozen base (original construction), and
//     Window B admits an advanced main only when it is a forward-only
//     descendant of the base that has touched none of the three authorized
//     B2B paths. A diverged main, or one that moved any authorized path, stays
//     RED. See `classifyProductMainPosition` for why a pin was wrong here.
//
...
/* Forward-only ancestry. Anything that prevents PROVING ancestry — an unknown
 * object, a broken repository, a git failure — answers false, so an
 * unprovable position is refused rather than admitted. */
...
/* ── Product-main lifecycle ──────────────────────────────────────────────── */

/* WHY THIS IS A WINDOW AND NOT A PIN.
 *
 * The original check required `origin/main === BASE_PRODUCT_MAIN`. That was
 * correct for exactly one moment — while the T02 candidate was being built —
 * and became false the instant Product main legitimately advanced on unrelated
 * work, turning a PERMANENT validator red for a reason that says nothing about
 * this change. Replacing one hash pin with a newer hash pin would just move the
 * same rot forward, so the position is classified by LIFECYCLE instead:
 *
 *   Window A — main is still exactly the frozen base. Original-construction
 *              semantics, unchanged and still exact.
 *   Window B — main has advanced, and is admissible ONLY when it is a
 *              forward-only descendant of the frozen base AND has not touched
 *              any of the three authorized B2B paths. That is precisely the
 *              condition under which the candidate's base remains a truthful
 *              statement about this change.
 *
 * Everything else stays RED: a main that diverged rather than advanced, and a
 * main that moved any authorized B2B path, both invalidate the candidate's
 * topology and MUST be detected.
 *
 * Kept as a pure function of stated facts so the negative controls can drive
 * it directly, rather than a check that can only ever observe whatever this
 * one checkout happens to be. */
function classifyProductMainPosition({ originMain, descendantOfBase, b2bPathsChanged }) {
  if (originMain === null) {
    /* No remote-tracking ref in this checkout. Ref movement is a push-time
     * property, so this is unobservable here rather than violated. */
    return { window: 'UNOBSERVABLE', admissible: true, reason: 'no origin/main ref in this checkout' };
  }
  if (originMain === BASE_PRODUCT_MAIN) {
    return { window: 'A', admissible: true, reason: 'Product main is still the frozen base' };
  }
  if (!descendantOfBase) {
    return {
      window: 'B',
      admissible: false,
      reason: `Product main ${originMain} is not a forward-only descendant of the frozen base`,
    };
  }
  if (b2bPathsChanged.length > 0) {
    return {
      window: 'B',
      admissible: false,
      reason: `Product main advanced across authorized B2B path(s): ${b2bPathsChanged.join(', ')}`,
    };
  }
  return {
    window: 'B',
    admissible: true,
    reason: 'Product main advanced forward-only and left every authorized B2B path untouched',
  };
}

/* The live facts, read once. */
function observeProductMainPosition() {
  const originMain = gitOrNull(['rev-parse', 'origin/main']);
  if (originMain === null) {
    return { originMain: null, descendantOfBase: false, b2bPathsChanged: [] };
  }
  const descendantOfBase = isAncestor(BASE_PRODUCT_MAIN, originMain);
  const b2bPathsChanged = (
    gitOrNull(['diff', '--name-only', BASE_PRODUCT_MAIN, originMain, '--', ...AUTHORIZED_PATHS]) || ''
  )
    .split('\n')
    .filter(Boolean);
  return { originMain, descendantOfBase, b2bPathsChanged };
}
...
/* The lifecycle window must stay DISCRIMINATING. Widening a check from one
 * exact hash to a window is exactly the kind of change that can quietly admit
 * everything, so both ways a Product main can invalidate this candidate are
 * driven through the same predicate the live check uses. Each control also
 * asserts that a clean Window-B position IS admitted, so a predicate that
 * simply refused everything could not pass them either. */

/* A forward-only main that left every authorized B2B path alone: the position
 * the window is meant to admit. Shared by both controls as the positive pole. */
const CLEAN_WINDOW_B = {
  originMain: '0'.repeat(40),
  descendantOfBase: true,
  b2bPathsChanged: [],
};

check('NEG_1 a later Product main touching an authorized B2B path stays RED', () => {
  assert.ok(
    classifyProductMainPosition(CLEAN_WINDOW_B).admissible,
    'the window must admit a clean forward-only main, or this control proves nothing'
  );
  for (const touched of AUTHORIZED_PATHS) {
    const verdict = classifyProductMainPosition({ ...CLEAN_WINDOW_B, b2bPathsChanged: [touched] });
    assert.equal(
      verdict.admissible,
      false,
      `a Product main that moved ${touched} must be refused`
    );
    assert.match(verdict.reason, /authorized B2B path/, 'the refusal must name the cause');
  }
  /* Window A is about the frozen base itself and cannot mask this: a main that
   * equals the base has no drift to inspect. */
  assert.equal(classifyProductMainPosition({ ...CLEAN_WINDOW_B, originMain: BASE_PRODUCT_MAIN }).window, 'A');
});

check('NEG_2 a Product main that is not a forward-only descendant stays RED', () => {
  assert.ok(
    classifyProductMainPosition(CLEAN_WINDOW_B).admissible,
    'the window must admit a clean forward-only main, or this control proves nothing'
  );
  const diverged = classifyProductMainPosition({ ...CLEAN_WINDOW_B, descendantOfBase: false });
  assert.equal(diverged.admissible, false, 'a diverged Product main must be refused');
  assert.match(diverged.reason, /forward-only descendant/, 'the refusal must name the cause');
  /* Non-descendancy is refused even when no B2B path moved, so an untouched
   * path set cannot rescue a rewritten history. */
  assert.equal(
    classifyProductMainPosition({
      originMain: 'f'.repeat(40),
      descendantOfBase: false,
      b2bPathsChanged: [],
    }).admissible,
    false
  );
  /* And the live ancestry helper is genuinely computed, not stubbed true. */
  assert.equal(isAncestor(BASE_PRODUCT_MAIN, BASE_PRODUCT_MAIN), true);
  assert.equal(isAncestor('0'.repeat(40), BASE_PRODUCT_MAIN), false);
});
...
if (!baseReachable) {
  ...
  /* Deliberately OUTSIDE the candidate-position branch. Window B exists for
   * assurance runs that are no longer positioned as the original candidate,
   * so a lifecycle check that skipped in exactly that position would be
   * unreachable where it matters most. */
  check('Product main is in an admissible B2B lifecycle window', () => {
    const facts = observeProductMainPosition();
    const verdict = classifyProductMainPosition(facts);
    assert.ok(
      verdict.admissible,
      `Product main position is inadmissible (window ${verdict.window}): ${verdict.reason}`
    );
    console.log(`       window ${verdict.window}: ${verdict.reason}`);
  });
