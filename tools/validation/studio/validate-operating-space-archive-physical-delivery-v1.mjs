...
if (!baseReachable) {
  ...
  if (!candidatePosition) {
    ...
    skip('Product main has not moved', 'HEAD is not positioned as the T02 candidate');
  } else {
    ...
    check('Product main has not moved', () => {
      const originMain = gitOrNull(['rev-parse', 'origin/main']);
      if (originMain === null) {
        /* No remote-tracking ref in this checkout: the local pin is still
         * checked, and ref movement is a push-time property. */
        assert.equal(gitOrNull(['cat-file', '-t', BASE_PRODUCT_MAIN]), 'commit');
        return;
      }
      assert.equal(originMain, BASE_PRODUCT_MAIN, 'Product main must remain at the pinned base during T02');
    });
  }
}
