...
### `H2O.Studio.platform.runtime` (URLs, env, navigation, loaded identity)
...
interface PlatformRuntime {
  ...
  getLoadedRuntimeIdentity(): Promise<LoadedRuntimeIdentity>;   // C5-C — implemented
  env: {
...
interface LoadedRuntimeIdentity {
  schema: 'h2o.studio.platform.loaded-runtime-identity.v1';
  adapter: 'fallback' | 'mv3' | 'tauri';
  available: boolean;               // at least one loaded fact was obtained
  reason: string | null;            // set only when available === false
  runtimeId: string | null;         // MV3: chrome.runtime.id · Tauri: app identifier
  displayName: string | null;       // MV3: manifest.name · Tauri: app name
  version: string | null;           // MV3: manifest.version · Tauri: app version
  versionName: string | null;       // MV3: manifest.version_name, opaque · Tauri: null
  desktopBuildIdentity: object | null;        // Tauri only: the governed native payload, untouched
  desktopBuildIdentityError: string | null;   // Tauri only: native transport failure, if any
}
```
...
**Loaded-runtime identity (C5-C):** `getLoadedRuntimeIdentity()` returns a
Promise on every adapter so MV3 and Tauri consumers share one flow. It reads
**loaded facts only** and derives nothing from them.

- **MV3 adapter** (`platform.mv3.js`) — reads `chrome.runtime.id` and the loaded
  manifest's `name`, `version` and `version_name` inside the adapter.
  `version_name` is passed through as an opaque loaded fact. `desktopBuildIdentity`
  is always `null` on MV3.
- **Tauri adapter** (`platform.tauri.js`) — reads the loaded app identifier /
  name / version through the already-granted `core:default` app commands and
  transports the existing native `h2o_studio_desktop_build_identity` payload as
  `desktopBuildIdentity`. Each seam fails on its own: a missing app fact is
  `null`; a failed native transport leaves `desktopBuildIdentity: null` and
  reports the failure in `desktopBuildIdentityError`.
- **Fallback adapter** — resolves `{ available: false, reason: 'runtime-identity-unavailable' }`
  with every fact `null`. It never synthesizes an id, name, version or build identity.

**Build & Delivery provenance fence.** The Desktop build-identity payload is
generated by the native binary under `L-DEVELOPER-BUILD-DELIVERY-SCOPE-STU`
authority (`build_identity.rs`, schema `h2o.studio.desktop-build-identity.v1`).
Host Integration is a transport only: the adapter hands the payload through
exactly as returned — it never normalizes, re-keys, defaults or regenerates
checkpoint, source commit, build timestamp, app version, architecture or
executable hash. Presentation-side schema validation (for example in
`settings/build-identity.panel.js`, which consumes this capability instead of
resolving Tauri internals itself) does not make the consumer a build authority.

Host Integration **must not infer** build channel, build variant, Lane identity,
BTP identity, role, source commit, checkpoint or any other provenance from
extension / runtime / display names. A name is echoed, never interpreted. When a
governed build fact is unavailable the value is `null` / unavailable; no
manifest-name heuristic may stand in for it.
...
interface PlatformFiles {
  ...
  importFile(opts?: {                                                      // C5-C — implemented
    mimeTypes?: string[];      // MV3 picker accept hint (e.g. 'application/json')
    extensions?: string[];     // picker filter, without the dot (e.g. 'json'); used by both hosts
    filterName?: string;       // Tauri dialog filter label
    title?: string;            // Tauri dialog title
  }): Promise<LocalFileRecord | null>;                                     // null = user cancelled
...
interface LocalFileRecord {
  schema: 'h2o.studio.platform.local-file.v1';
  name: string;   // user-facing file name (MV3: File.name · Tauri: last path segment)
  size: number;   // bytes (MV3: File.size · Tauri: byte length actually read)
  type: string;   // host-reported MIME type, or '' when the host reports none
  text: string;   // the file's UTF-8 contents, already read by the adapter
}
```

**Implementation status:**

- `exportBlob` — **implemented** on MV3 + Tauri (Phase 3a / 3c-B).
- `importFile` — **implemented** on MV3 + Tauri (C5-C). First slice: **one
  UTF-8 text file**. Binary / multi-file import is not part of the contract.
- `exportJson`, `importJson` — **placeholders**. `importJson` is intentionally
  not activated: it would carry Saved Chats parsing semantics that do not
  belong in an adapter.

**`importFile` (C5-C):** Host Integration owns the whole pick-and-read
mechanic — picker invocation, host-specific file selection, the host-side
UTF-8 text read and normalization. Feature code receives the already-read
`text` plus host-neutral `name` / `size` / `type` facts and never branches on
`chrome.*`, Tauri internals, browser `File` objects, Tauri paths, file inputs
or OS dialog APIs. Requires a user gesture. Single file only.

- **MV3 adapter** (`platform.mv3.js`) — creates a transient hidden
  `<input type="file">` (single, `accept` from `mimeTypes` / `extensions`),
  clicks it, reads the chosen `File` with `File.text()` (or `FileReader`),
  and removes the input once settled. The browser `File` never leaves the
  adapter.
- **Tauri adapter** (`platform.tauri.js`) — `plugin:dialog|open`
  (`multiple: false`, `directory: false`, filters from `extensions`) then
  `plugin:fs|read_text_file` on the chosen path (the dialog plugin scopes the
  picked file; byte-array read responses are decoded as UTF-8). `name` is the
  path's last segment; the path itself never leaves the adapter. `type` is
  `''` because the OS dialog reports no MIME and none is guessed. Both
  commands are already granted (`dialog:allow-open`,
  `fs:allow-read-text-file`) — **no new Rust command, plugin dependency or
  capability grant**.
- **Fallback adapter** — `importFile` is always present and **rejects** with
  `platform.files.importFile unavailable (no adapter bound)`. Unavailability is
  an error; cancellation is not.

**Cancellation is non-error.** A dismissed picker (MV3 `cancel` event or a
change event without a file; Tauri `null` from the dialog) resolves `null`.
A failed read rejects with the host error and is never masked as a cancel.

**Saved Chats semantic fence.** `importFile` delivers text and nothing more.
JSON parsing, full-bundle recognition, `h2o.studio.fullBundle.v2` / legacy
archive schema decisions, migration validation, dry-run semantics,
backup-before-import, import mode and archive write sequencing remain
C5-B / Saved Chats responsibilities in the caller. `exportBlob` likewise only
delivers bytes the caller has already serialized.
...
- **Fallback adapter** — `files: { available: false, importFile: <rejects> }`.
  Callers MUST feature-detect
  `platform.files.available && typeof platform.files.exportBlob === 'function'`
  before calling `exportBlob`, OR provide their own inline fallback (see
